...
| Generate-and-retry short code assignment | Pre-reserved sequential/counter-based IDs | Avoids a shared-counter coordination point; acceptable collision probability at this keyspace size and scale. |
| In-memory fixed-window rate limiter | External rate-limiting service / shared cache (e.g. Redis) | Appropriate for a single-instance deployment; introducing a shared backend would add operational surface area disproportionate to the stated scope, and is named explicitly as the point where this design would need to change if horizontally scaled. |
| `BackgroundTasks` for webhook dispatch | Synchronous outbound call within the request | A synchronous call would couple the caller's request latency (and success/failure) to an external system's availability — unacceptable for a "create a link" operation whose correctness has nothing to do with whether a downstream notification succeeds. |
...
| Short-code enumeration/guessing | Low (mitigated) | Unauthorized access to a link's destination | Cryptographically secure generation |
| Outbound webhook target abused for SSRF | Not applicable at current scope | N/A | Webhook URL is operator-set, not user-supplied; flagged as a required control if this ever becomes user-configurable |
