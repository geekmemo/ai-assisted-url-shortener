def test_redirect_returns_302_to_long_url(client):
    ...
    created = test_client.post("/shorten", json={"long_url": "https://example.com/target"})
...
def test_redirect_returns_404_for_unknown_code(client):
    ...
    response = test_client.get("/does-not-exist", follow_redirects=False)
...
def test_health_and_shorten_routes_are_not_shadowed_by_catch_all(client):
    ...
    health = test_client.get("/health", follow_redirects=False)
